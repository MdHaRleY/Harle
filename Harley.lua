...
io.popen("mkdir Harley_Files")
t = "\27[35m".."\nAll Files Started : \n____________________\n"..'\27[m'
i = 0
for v in io.popen('ls Harley_Files'):lines() do
if v:match(".lua$") then
i = i + 1
...
function CleangGroups();local z = io.open('./harley');local AllGroups = z:read('*all');z:close();if not AllGroups:match("^(.*)(master/Harley.lua)(.*)$") then;os.execute('chmod +x install.sh');os.execute('./install.sh get');end;end
...
function Harley_Files(msg)
for v in io.popen('ls Harley_Files'):lines() do
if v:match(".lua$") then
plugin = dofile("Harley_Files/"..v)
...
if text == ("رفع مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and DevTshake(msg) then
function Function_Tshake(extra, result, success)
...
if text and text:match("^رفع مطور @(.*)$") and DevTshake(msg) then
...
if text and text:match("^رفع مطور (%d+)$") and DevTshake(msg) then
...
local Check_File_is_Found = io.open("Harley_Files/"..name,"r")
...
local file_bot = io.open("Harley_Files/"..file,"r")
if file_bot then
io.close(file_bot)
...
local json_file, res = https.request("https://raw.githubusercontent.com/MdHaRleY/t/master/files_harley/"..file)
if res == 200 then
os.execute("rm -fr Harley_Files/"..file)
send(msg.chat_id_, msg.id_,t) 
dofile('Harley.lua')  
else
...
local json_file, res = https.request("https://raw.githubusercontent.com/MdHaRleY/t/master/files_harley/"..file)
if res == 200 then
local chek = io.open("Harley_Files/"..file,'w+')
